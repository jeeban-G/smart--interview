...
// OpenClaw 配置
const OPENCLAW_API_KEY = process.env.OPENCLAW_API_KEY || 'openclaw';
const OPENCLAW_API_URL = process.env.OPENCLAW_API_URL || 'http://localhost:18789/v1/chat/completions';
const OPENCLAW_MODEL = process.env.OPENCLAW_MODEL || 'deepseek-chat';
...
export async function callOpenClawAPI(
  ...
  try {
    ...
    const text = response.data.choices?.[0]?.message?.content || '抱歉，我暂时无法回答这个问题。';

    return { content: text };
  } catch (error: any) {
    console.error('[OpenClaw Service] API error:', error?.response?.data || error?.message || error);
    return {
...
export const openclawService = {
  ...
  async generateResponse(
    ...
    return callOpenClawAPI(systemPrompt, userPrompt);
  },
...
要求：做一个简洁的自我介绍，60字以内，突出自己的特点和优势，自然像和真人聊天。`;
    ...
    return callOpenClawAPI(introPrompt, '请做自我介绍', 300);
  },
...
请生成面试官的问题，要简短、具体、有针对性。符合${company}的面试风格。`;
    ...
    return callOpenClawAPI(systemPrompt, '请生成面试官的问题', 512);
  },
...
请生成候选人的回答，要自然、具体、有个人特色。`;
    ...
    return callOpenClawAPI(systemPrompt, '请生成候选人的回答', 512);
  },
